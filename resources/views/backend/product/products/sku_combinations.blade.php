			...
			@foreach ($combinations as $key => $combination)
				...
				@if(strlen($str) > 0)
					<tr class="variant">
						...
						<td>
							<input type="number" lang="en" name="gold_rate_{{ $str }}" value="{{ $gold_rate }}" min="0" step="0.01"
								class="form-control gold-rate" readonly required>
						</td>
						<td>
							<input type="number" lang="en" name="gold_qty_{{ $str }}" value="{{ $gold_qty }}" min="1" step="1"
								class="form-control gold-qty" required>
						</td>
						<td>
							<input type="number" lang="en" name="diamond_price_{{ $str }}" value="{{ $diamond_price }}" min="0"
								step="0.01" class="form-control diamond-price" required>
						</td>
						<td>
							<input type="number" lang="en" name="price_{{ $str }}" value="{{ $unit_price }}" min="0" step="0.01"
								class="form-control variant-price" readonly required>
						</td>
						<td>
